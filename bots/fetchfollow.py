        ...
        if len(user_tweets) > 0:
            ...
            for tweet in user_tweets:
                ...
                if (len(tags & lower_given_tag) > 0) or found_tag_in_text:
                    ...
                    if not tweet.favorited and (random.random() > 0.5) and tweet.lang == "en":
...
def score_user(user):
    ...
    followers = _utils.scaled_sigmoid(
        x=-user.followers_count, stretch=200, max_score=100, center=300
    )
...
def get_users_from_recent_tweets(cnt=10, hashtags=None):
    ...
    for tweet in tweepy.Cursor(
        get_twitter_api().search_tweets, q=q, lang="en", count=cnt, result_type="recent"
    ).items(cnt):
...
def fetchfollow(event=None, context=None):
    ...
    tweepy.User.__hash__ = lambda self: hash(self.id_str)
...
def comment_tweet(user, tweet):
    ...
    logger.info(f"Commenting: {tweet.id} with: {comment}")
    get_twitter_api().update_status(
        comment, in_reply_to_status_id=tweet.id_str, auto_populate_reply_metadata=True
    )
